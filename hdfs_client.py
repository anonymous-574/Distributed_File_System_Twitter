...
class HDFSClient:
    def __init__(self, hdfs_url, server_id):
        ...
        self.logger.info(f"NameNode: {self.namenode_host}:{self.namenode_port}")
            ...
            try:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        self.logger.warning(f"  NameNode Web UI returned status: {response.status_code}")
                except requests.exceptions.RequestException as e:
                    self.logger.warning(f"  NameNode Web UI not accessible yet: {e}")
                ...
                try:
                    ...
                    if "already exists" in str(e).lower():
                        ...
                        self.logger.warning(f"  Could not create test directory: {e}")
                ...
                try:
                    ...
                    self.logger.warning(f"  HDFS read/write test failed: {e}")
        ...
        try:
            ...
            if self.hdfs:
                ...
                try:
                    ...
                    self.logger.warning(f"  Could not verify post storage: {e}")

                return True
            else:
                self.logger.warning("  HDFS not available, using local storage fallback")
        ...
        try:
            ...
            if self.hdfs:
                try:
                    ...
                    self.logger.warning(f"  Could not access HDFS posts directory: {e}")
                    posts = self.get_local_posts()
            else:
                self.logger.warning(" HDFS not available, using local fallback")
                posts = self.get_local_posts()
        ...
        try:
            ...
            if self.hdfs:
                try:
                    ...
                    self.logger.warning(f"  Could not access HDFS comments directory: {e}")
        ...
        try:
            ...
            if self.hdfs:
                try:
                    ...
                    self.logger.warning(f"  Could not get HDFS post count: {e}")
